...
import { BWICK_API_URL, BWICK_TESTNET_API_URL } from 'src/env/config';
...
export function createHeaders(options: Options) {
  return {
    ...
    'Bwick-Client-Type': platform,
    'Bwick-Client-Version': version,
...
const resolveUrl = (input: UrlInput): string | URL => {
  if ('url' in input) {
    ...
    const base = source === 'testnet' ? BWICK_TESTNET_API_URL : BWICK_API_URL;
    invariant(
      base,
      `One of API URLs not found in env: ${BWICK_TESTNET_API_URL}, ${BWICK_API_URL}`
    );
    return createUrl({ base, pathname: endpoint });
  }
};

export class BwickHttpClient {
