import { BwickAPI } from '../bwick-api/bwick-api.client';
import type { Identity } from '../bwick-api/requests/wallet-get-meta';
...
/** TODO:
 * parameterize apiClient: BwickApiClient as a dependency
...
export async function lookupAddressNames(address: string): Promise<string[]> {
  try {
    const response = await BwickAPI.getWalletsMeta({ identifiers: [address] });
    return (
...
export async function resolveDomain(domain: string): Promise<string | null> {
  try {
    const response = await BwickAPI.getWalletsMeta({ identifiers: [domain] });
